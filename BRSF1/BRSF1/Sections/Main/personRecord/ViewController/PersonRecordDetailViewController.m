//
//  PersonRecordDetailViewController.m
//  BRSF
//
...
#import "PersonRecordDetailViewController.h"
...
@interface PersonRecordDetailViewController ()<UITextViewDelegate,UINavigationControllerDelegate>

@end

@implementation PersonRecordDetailViewController
...
- (void)viewDidLoad {
    ...
    NSString *path = [doc stringByAppendingPathComponent:@"PATIENT.sqlite"];
    NSLog(@"path===%@",path);
    self.dbPath = path;
    ...
    [_mysendButton setTitle:@"修改" forState:UIControlStateNormal];
    [_mysendButton addTarget:self action:@selector(editNewUserInfo) forControlEvents:UIControlEventTouchUpInside];
...
- (void)readNewUserInfo
{
    ...
    if ([db open]) {
        ...
        if (_txtView.text.length == 0||_titleTextField.text.length == 0){
            ...
            [XFNotices noticesWithTitle:@"请输入填写信息" Time:1.5 View:self.view Style:(XFNoticesStyleFail)];
        }else{
            ...
            if (self.operateType == 0){//执行插入操作
                ...
                sql = @"select txt from PERSONRECORD WHERE title = ?";
                //                sql = @"INSERT INTO PERSONRECORD (createtime,title,txt) VALUES (?,?,?) ";
            }
            NSLog(@"_txtView==%@", _txtView.text);
            NSString *res = [db executeQuery:sql,_titleTextField,_txtView.text];
            
            if (!res) {
                [XFNotices noticesWithTitle:@"数据查询失败" Time:1.5 View:self.view Style:(XFNoticesStyleFail)];
            }else{
                [XFNotices noticesWithTitle:@"数据查询成功" Time:1.5 View:self.view Style:(XFNoticesStyleFail)];
            }
        }
    }else{
        NSLog(@"数据库打开失败") ;
...
//                sql = @"INSERT INTO PERSONRECORD (createtime,title,txt) VALUES (?,?,?) ";
            ...
            if (!res) {
                [XFNotices noticesWithTitle:@"数据更新错误" Time:1.5 View:self.view Style:(XFNoticesStyleFail)];
            }else{
                [XFNotices noticesWithTitle:@"数据更新成功" Time:1.5 View:self.view Style:(XFNoticesStyleFail)];
            }
        }
    }else{
        NSLog(@"数据库打开失败") ;
